...
def main():
    ...
    print("OpenCV backend:", cv2.cuda.getCudaEnabledDeviceCount())
    ...
    while True:
        ...
        if cv2.cuda.getCudaEnabledDeviceCount() > 0:
            # Create GPU versions of images
            gpu_image = cv2.cuda_GpuMat()
            gpu_image.upload(image)
            
            # Perform operations on GPU
            gpu_image_with_landmarks = detect_hand_landmarks(gpu_image)
            
            # Download result from GPU
            image_with_landmarks = gpu_image_with_landmarks.download()
        else:
